...
class CarService(models.Model):
    ...
    def __str__(self):
        return f'{self.car_model.brand} {self.car_model.model} {self.car_model.engine} - {self.service_price.service.name} {self.service_price.price} eur'

    def get_absolute_url(self):
        return reverse('carservice-detail', args=[str(self.id)])
...
class Car(models.Model):
    ...
    car_model = models.ForeignKey('CarModel', on_delete=models.SET_NULL, null=True)
    license_plate = models.CharField('License plate', max_length=7, help_text='3 letters and 3 numbers with a space in between'
    ...
    vin_code = models.CharField('VIN code', max_length=17, help_text='eg. 1HGCM82633A004352 <a href="https://www.autodna.lt/vin-numeris">VIN code info</a>')
...
class Order(models.Model):
    ...
    def __str__(self):
        return f'{self.client_car.license_plate} - {self.price} eur'
...
class OrderLine(models.Model):
    ...
    service = models.ForeignKey('Service', on_delete=models.SET_NULL, null=True)
    amount = models.CharField('Amount', max_length=10, help_text='Amount')
    price = models.CharField('Price', max_length=10, help_text='Price of repair')

    def __str__(self):
        return f'{self.order.client_car.license_plate} {self.service.name} - {self.amount} time(s) {self.price} eur'
